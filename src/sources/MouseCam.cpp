/*
	MouseCam code shamelessly swiped from Be Inc. source code project Magnify
	and bolted onto BePhotoMagic
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static const char *kPrefsFileName = "BPMMouseCam_prefs";
...
TWindow::TWindow(int32 pixelCount)
	: BWindow( BRect(0,0,0,0), SpTranslate("MouseCam"), B_FLOATING_WINDOW,
	B_OUTLINE_RESIZE | B_NOT_MINIMIZABLE | B_NOT_ZOOMABLE |
...
TWindow::~TWindow()
{
	BMessage *msg=new BMessage(MOUSECAM_CLOSED);
...
//		pixel count
//		pixel size
const char* const kAppName = "MouseCam";
...
void
TWindow::FrameMoved(BPoint origin)
{
	BMessage *msg=new BMessage(CHILDWIN_MOVED);
	msg->AddPoint("mousewin",origin);
	be_app->PostMessage(msg);
}
...
{
	BRect r(0,0,375,240);
	BWindow* w = new BWindow(r, "MouseCam Help", B_TITLED_WINDOW,
...
BuildInfoMenu(BMenu *menu)
{
	...
	menuItem = new BMenuItem("Copy Image", new BMessage(msg_copy_image));
	menu->AddItem(menuItem);
	menuItem = new BMenuItem("Save Image", new BMessage(msg_save));
	menu->AddItem(menuItem);
	menu->AddSeparatorItem();

	menuItem = new BMenuItem("Show/Hide Info", new BMessage(msg_show_info));
	menu->AddItem(menuItem);
	menuItem = new BMenuItem("Show/Hide Grid", new BMessage(msg_toggle_grid));
	menu->AddItem(menuItem);
	menu->AddSeparatorItem();
	menuItem = new BMenuItem("Add a Crosshair", new BMessage(msg_add_cross_hair));
	menu->AddItem(menuItem);
	menuItem = new BMenuItem("Remove a Crosshair", new BMessage(msg_remove_cross_hair));
	menu->AddItem(menuItem);
	menu->AddSeparatorItem();

	menuItem = new BMenuItem("Freeze/Unfreeze image", new BMessage(msg_freeze));
	menu->AddItem(menuItem);
